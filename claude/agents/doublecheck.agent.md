...
## Pre-PR Quality Gate Sequence

Run these phases in order before any PR. Stop and fix at the first failure.

### Phase 1: Build
```bash
npm run build 2>&1 | tail -20       # JS/TS
...
```bash
grep -rn "sk-\|api_key\|password" --include="*.ts" --include="*.py" src/ 2>/dev/null | head -10
```
